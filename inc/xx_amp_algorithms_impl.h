...
#define AMP_ALGORITHMS_ENUMS
    ...
    namespace _details
    {
        ...
        decltype(auto) reduce(const concurrency::accelerator_view &accl_view, const IndexableInputView& input_view, BinaryFunction&& binary_op)
        {
            ...
            using result_type = std::remove_cv_t<typename IndexableInputView::value_type>;
            // runtime sizes
			...
			return std::accumulate(amp_stl_algorithms::cbegin(global_buffer_view) + 1,
								   amp_stl_algorithms::cend(global_buffer_view),
								   *amp_stl_algorithms::cbegin(global_buffer_view),
								   std::forward<BinaryFunction>(binary_op));
        }
        ...
        static constexpr int scan_default_tile_size = 512;

        template <int TileSize, typename _BinaryOp, typename T>
        inline T scan_tile_exclusive(T* const tile_data, const concurrency::tiled_index<TileSize>& tidx, _BinaryOp&& op) restrict(amp)
        {
        ...
        inline void scan(const concurrency::accelerator_view& accl_view, const concurrency::array_view<T>& input_view, const concurrency::array_view<T>& output_view, _BinaryFunc&& op)
        {
            ...
            concurrency::parallel_for_each(accl_view, compute_domain, [=](auto&& tidx) restrict(amp)
            {
            ...
            {
                scan<TileSize, amp_algorithms::scan_mode::exclusive>(accl_view, tile_results_vw, tile_results_vw, std::forward<_BinaryFunc>(op));
            }
            else
            {
                concurrency::parallel_for_each(accl_view, compute_domain, [=](concurrency::tiled_index<TileSize> tidx) restrict(amp)
                {
                    ...
                    _details::scan_tile_exclusive<TileSize>(tile_data, tidx, amp_algorithms::plus<>());
        ...
        template <typename T>
        inline T segment_exclusive_scan(const array_view<T, 1>& exclusive_scan, int segment_width, int i) restrict(amp, cpu)
        {
        ...
        inline int radix_key_value(const T value, unsigned key_idx) restrict(amp, cpu)
        {
        ...
        template <typename T>
        inline void initialize_bins(T* const bin_data, int bin_count) restrict(amp)
        {
        ...
        void radix_sort_tile_by_key(T* const tile_data, int data_size, concurrency::tiled_index<tile_size> tidx, int key_idx) restrict(amp)
        {
            constexpr unsigned bin_count = 1 << tile_key_bit_width;
            const int gidx = tidx.global[0];
            ...
            if (idx == 0)
            {
                ...
                const int tile_data_size = amp_stl_algorithms::min(tile_size, (data_size - (tlx * tile_size)));
            ...
            _details::scan_tile_exclusive<tile_size>(tile_radix_values, tidx, amp_algorithms::plus<>());
        ...
        void radix_sort_by_key(const concurrency::accelerator_view& accl_view, const concurrency::array_view<T>& input_view, concurrency::array_view<T>& output_view, int key_idx)
        {
            static constexpr unsigned type_width = sizeof(T) * CHAR_BIT;
            static constexpr int bin_count = 1 << key_bit_width;
            ...
            concurrency::parallel_for_each(accl_view, compute_domain, [=, &per_tile_rdx_offsets, &tile_histograms, &global_rdx_offsets](concurrency::tiled_index<tile_size> tidx) restrict(amp)
            {
                ...
                constexpr int keys_per_tile = (key_bit_width / tile_key_bit_width);
        ...
        {
            static constexpr int key_count = bit_count<T>() / key_bit_width;
