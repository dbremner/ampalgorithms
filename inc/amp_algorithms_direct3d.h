...
namespace amp_algorithms
{
    ...
    namespace direct3d
    {
        ...
        public:
            ...
            bitvector(const unsigned scan_size) : m_scan_size(scan_size)
            {
                data = std::vector<unsigned>(bits_pad_to_uint(scan_size), 0);
            }
            ...
            void initialize(Func pred)
            {
                ...
                for (unsigned idx = 0; idx < data.size() && flag_counter < m_scan_size; ++idx)
                {
                    unsigned bag_of_bits = data[idx];
                    for (unsigned offset = 0; offset < amp_algorithms::bit_count<unsigned>() && flag_counter < m_scan_size; ++offset)
                    {
